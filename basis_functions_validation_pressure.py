...
xvg_rr1_k0 = np.loadtxt('energy_press_ref0rr1_k0.txt')
xvg_rr2_k0 = np.loadtxt('energy_press_ref0rr2_k0.txt')
xvg_rr3_k0 = np.loadtxt('energy_press_ref0rr3_k0.txt')
xvg_rr4_k0 = np.loadtxt('energy_press_ref0rr4_k0.txt')

xvg_rr_noLJ = np.loadtxt('energy_press_ref0rr_noLJ.txt')
P_noLJ = xvg_rr_noLJ[:,4]
VirXX_noLJ = xvg_rr_noLJ[:,5]
VirYY_noLJ = xvg_rr_noLJ[:,6]
VirZZ_noLJ = xvg_rr_noLJ[:,7]
Vir_noLJ = (VirXX_noLJ + VirYY_noLJ + VirZZ_noLJ)/3.

xvg_rr1_decoupled = xvg_rr1 - xvg_rr1_k0 - xvg_rr_noLJ
xvg_rr2_decoupled = xvg_rr2 - xvg_rr2_k0 - xvg_rr_noLJ 
xvg_rr3_decoupled = xvg_rr3 - xvg_rr3_k0 - xvg_rr_noLJ
xvg_rr4_decoupled = xvg_rr4 - xvg_rr4_k0 - xvg_rr_noLJ
...
Vir_basis_2 = np.zeros([2,n_snaps])
VirXX_basis_2 = np.zeros([2,n_snaps])
VirYY_basis_2 = np.zeros([2,n_snaps])
VirZZ_basis_2 = np.zeros([2,n_snaps])
Pdc_basis = np.zeros([2,n_snaps])
P_basis = np.zeros([2,n_snaps])
P_basis_2 = np.zeros([2,n_snaps])
...
#Virial XX,YY,ZZ
VirXX_basis[0] = xvg_rr1_k0[:,5]
VirXX_basis[1] = xvg_rr2_k0[:,5]
VirYY_basis[0] = xvg_rr1_k0[:,6]
VirYY_basis[1] = xvg_rr2_k0[:,6]
VirZZ_basis[0] = xvg_rr1_k0[:,7]
VirZZ_basis[1] = xvg_rr2_k0[:,7]

#Virial XX,YY,ZZ
VirXX_basis_2[0] = xvg_rr1_decoupled[:,5]
VirXX_basis_2[1] = xvg_rr2_decoupled[:,5]
VirYY_basis_2[0] = xvg_rr1_decoupled[:,6]
VirYY_basis_2[1] = xvg_rr2_decoupled[:,6]
VirZZ_basis_2[0] = xvg_rr1_decoupled[:,7]
VirZZ_basis_2[1] = xvg_rr2_decoupled[:,7]
   
#Pressure XX,YY,ZZ
PXX_basis[0] = xvg_rr1_k0[:,8]
PXX_basis[1] = xvg_rr2_k0[:,8]    
PYY_basis[0] = xvg_rr1_k0[:,9]  
PYY_basis[1] = xvg_rr2_k0[:,9]
PZZ_basis[0] = xvg_rr1_k0[:,10]
PZZ_basis[1] = xvg_rr2_k0[:,10] 
         
P_basis[0] = xvg_rr1_k0[:,4]
P_basis[1] = xvg_rr2_k0[:,4]

P_basis_2[0] = xvg_rr1_decoupled[:,4]
P_basis_2[1] = xvg_rr2_decoupled[:,4]

#KE is constant with force field
KE = xvg_rr1[:,2]

plt.plot(VirXX_basis[0,:],'ro')
plt.plot(VirXX_basis[1,:],'bx')
plt.plot(VirYY_basis[0,:],'ro')
plt.plot(VirYY_basis[1,:],'bx')
plt.plot(VirZZ_basis[0,:],'ro')
plt.plot(VirZZ_basis[1,:],'bx')
plt.show()
...
for ibasis in range(2):
    ...
    Vir_basis_2[ibasis] = (VirXX_basis_2[ibasis]+VirYY_basis_2[ibasis]+VirZZ_basis_2[ibasis])/3.
...
# Let's try determining the virial contributions directly from the pressures and KE instead of from each component (this way I don't need to change my rerun codes almost at all)   
...
Vir_basis_alt_2 = -KE/3.-P_basis_2/2.*Vbox/nm3tom3/kJm3tobar*NA #Curretnly my code doulbe counts the KE, I will want to do this differently but this is a proof of concept
...
for rr_i, rr_new in enumerate(rr_all):
    ...
    if rr_new == 3:
        ...
        U_new = xvg_rr3_k0[:,0]
        VirXX_new = xvg_rr3_k0[:,5]
        VirYY_new = xvg_rr3_k0[:,6]
        VirZZ_new = xvg_rr3_k0[:,7]
        VirXX_new_2 = xvg_rr3_decoupled[:,5]
        VirYY_new_2 = xvg_rr3_decoupled[:,6]
        VirZZ_new_2 = xvg_rr3_decoupled[:,7]
        Pdc_new = xvg_rr3[:,3]
        PXX_new = xvg_rr3[:,8]
        PYY_new = xvg_rr3[:,9]
        PZZ_new = xvg_rr3[:,10]
        ...
        U_new = xvg_rr4_k0[:,0]
        VirXX_new = xvg_rr4_k0[:,5]
        VirYY_new = xvg_rr4_k0[:,6]
        VirZZ_new = xvg_rr4_k0[:,7]
        VirXX_new_2 = xvg_rr4_decoupled[:,5]
        VirYY_new_2 = xvg_rr4_decoupled[:,6]
        VirZZ_new_2 = xvg_rr4_decoupled[:,7]
        Pdc_new = xvg_rr4[:,3]
        PXX_new = xvg_rr4[:,8]
        PYY_new = xvg_rr4[:,9]
        PZZ_new = xvg_rr4[:,10]
    ...
    Vir_new_2 = (VirXX_new_2 + VirYY_new_2 + VirZZ_new_2)/3. #Dividing by 3 is an important step
    ...
    rdrarray = np.linalg.solve(Cmatrix,Vir_basis) #First entry of rdrarray is the sum of r*d(r^-lambda)/dr and second entry is sum of r*d(r^-6)/dr
    print(rdrarray)
    
    rdrarray_2 = np.linalg.solve(Cmatrix,Vir_basis_2) #First entry of rdrarray is the sum of r*d(r^-lambda)/dr and second entry is sum of r*d(r^-6)/dr
    print(rdrarray_2)
    
    rdrXXarray = np.linalg.solve(Cmatrix,VirXX_basis) #First entry of rdrarray is the sum of r*d(r^-lambda)/dr and second entry is sum of r*d(r^-6)/dr
    rdrYYarray = np.linalg.solve(Cmatrix,VirYY_basis) #First entry of rdrarray is the sum of r*d(r^-lambda)/dr and second entry is sum of r*d(r^-6)/dr
    rdrZZarray = np.linalg.solve(Cmatrix,VirZZ_basis) #First entry of rdrarray is the sum of r*d(r^-lambda)/dr and second entry is sum of r*d(r^-6)/dr

    rdrXXarray_2 = np.linalg.solve(Cmatrix,VirXX_basis_2) #First entry of rdrarray is the sum of r*d(r^-lambda)/dr and second entry is sum of r*d(r^-6)/dr
    rdrYYarray_2 = np.linalg.solve(Cmatrix,VirYY_basis_2) #First entry of rdrarray is the sum of r*d(r^-lambda)/dr and second entry is sum of r*d(r^-6)/dr
    rdrZZarray_2 = np.linalg.solve(Cmatrix,VirZZ_basis_2) #First entry of rdrarray is the sum of r*d(r^-lambda)/dr and second entry is sum of r*d(r^-6)/dr
    ...
    for ibasis in range(2):
        ...
        assert np.all(np.abs(VirXXlam+VirXX6 - VirXX_basis[ibasis]) < 1e-6), 'Pressures do not add up'
    ...
    Vir_new_hat = np.linalg.multi_dot([Cmatrix_new,rdrarray])
    #print('Predicted virial: '+str(Vir_new_hat))
    #print('Actual virial is '+str(Vir_new))
    
    Vir_new_hat_2 = np.linalg.multi_dot([Cmatrix_new,rdrarray_2])
    ...
    P_new_hat = 2./Vbox*(KE/3.-(Vir_new_hat+Vir_new_hat_2+Vir_noLJ))*nm3tom3*kJm3tobar/NA
    
    VirXX_new_hat = np.linalg.multi_dot([Cmatrix_new,rdrXXarray])
    #print('Predicted virial-XX: '+str(VirXX_new_hat))
    #print('Actual virial-XX is '+str(VirXX_new))
    
    VirYY_new_hat = np.linalg.multi_dot([Cmatrix_new,rdrYYarray])
    #print('Predicted virial-YY: '+str(VirYY_new_hat))
    #print('Actual virial-YY is '+str(VirYY_new))
    
    VirZZ_new_hat = np.linalg.multi_dot([Cmatrix_new,rdrZZarray])
    ...
    VirXX_new_hat_2 = np.linalg.multi_dot([Cmatrix_new,rdrXXarray_2])
    VirYY_new_hat_2 = np.linalg.multi_dot([Cmatrix_new,rdrYYarray_2])
    VirZZ_new_hat_2 = np.linalg.multi_dot([Cmatrix_new,rdrZZarray_2])
    
    Vir_new_hat_alt_2 = (VirXX_new_hat_2 + VirYY_new_hat_2 + VirZZ_new_hat_2)/3.
                      
    P_new_hat_alt = 2./Vbox*(KE/3.-(Vir_new_hat_alt+Vir_new_hat_alt_2+Vir_noLJ))*nm3tom3*kJm3tobar/NA
    ...
    dev_Vir_2[rr_i] = Vir_new_hat_2 - Vir_new_2
    dev_P[rr_i] = P_new_hat_alt - P_new
    
    mean_dev_U[rr_i] = np.mean(dev_U[rr_i])
    mean_dev_Vir[rr_i] = np.mean(dev_Vir[rr_i])
    mean_dev_Vir_2[rr_i] = np.mean(dev_Vir_2[rr_i])
    ...
    per_bias_Vir_2[rr_i] = mean_dev_Vir_2[rr_i] / np.mean(Vir_new_2) * 100.
    ...
    Vir_parity_2 = np.array([np.min(Vir_new_2),np.max(Vir_new_2)])
    ...
    plt.plot(Vir_new_2,Vir_new_hat_2,'ro')
    #plt.plot(Vir_new,Vir_new_hat_alt,'bx')
    plt.plot(Vir_parity_2,Vir_parity_2,'k--')
    plt.xlabel('Simulated Virial (kJ/mol)')
    plt.ylabel('Basis Function Virial (kJ/mol)')
    plt.show()
    ...
    print('The average deviation in virial is '+str(mean_dev_Vir_2[rr_i])+' (kJ/mol), i.e. '+str(per_bias_Vir_2[rr_i])+'% bias')
...
plt.plot(dev_Vir_2[0],'ro',label='rr3')
plt.plot(dev_Vir_2[1],'bx',label='rr4')
plt.xlabel('Snapshot')
plt.ylabel('Deviation in virial (kJ/mol)')
plt.legend()
plt.show()
