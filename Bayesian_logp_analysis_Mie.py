...
def analyze_ITIC(iRerun,use_PCFR,metric,smoothed=True): 
    ...
    if use_PCFR:
        ...
        USim, dUSim, PSim, dPSim, ZSim, Z1rhoSim, NeffSim = np.loadtxt('MBAR_rr'+str(iRerun),skiprows=1,unpack=True)
    Tsat, rhoLSim, PsatSim, rhovSim = np.loadtxt('ITIC_'+str(iRerun),skiprows=1,unpack=True)

    logp = np.loadtxt('logp')
    ...
    if np.all(Tsat>RP_TC):

        return 300, 30, 100, 700, 1300, 4, 1.
...
#    f.close()
    ...
    if metric == 'SSE':
        return SSErhoL, SSEPsat, SSErhov, SSEU, SSEP, SSEZ, Navg, logp[iRerun]
    elif metric == 'RMS':
        return RMSrhoL, RMSPsat, RMSrhov, RMSU, RMSP, RMSZ, Navg, logp[iRerun]
...
def print_figures(opt_type,use_PCFR,metric):
    ...
    if opt_type == 'scan': #This doesn't work with basis functions or with multiple references
        ...
        logp = np.empty([neps,nsig]) 
    
        iRerun = 0
        for ieps in range(neps):
            for isig in range(nsig):
                SSErhoL[ieps,isig], SSEPsat[ieps,isig], SSErhov[ieps,isig], SSEU[ieps,isig], SSEP[ieps,isig], SSEZ[ieps,isig], Neffavg[ieps,isig],logp[ieps,isig] = analyze_ITIC(iRerun,use_PCFR,metric)
                iRerun += 1

        logp_scaled = logp - np.max(logp)
        assert np.abs(np.max(logp_scaled)) < 1e-3, 'The maximum likelihood value does not have a logp of 0'
        prob = np.exp(logp_scaled)
        ...
        f = plt.figure()
        plt.contour(sig,eps,logp)
        plt.ylabel('$\epsilon$ (K)')
        plt.xlabel('$\sigma$ (nm)')
        plt.ylim([min(eps),max(eps)])
        plt.xlim([min(sig),max(sig)])
        plt.title(r'Log(Posterior)')
        plt.colorbar()
        f.savefig(compound+'ref_'+str(iRef)+'_logp.pdf')

        f = plt.figure()
        plt.contour(sig,eps,prob)
        plt.ylabel('$\epsilon$ (K)')
        plt.xlabel('$\sigma$ (nm)')
        plt.ylim([min(eps),max(eps)])
        plt.xlim([min(sig),max(sig)])
        plt.title(r'Pseudo-probability')
        plt.colorbar()
        f.savefig(compound+'ref_'+str(iRef)+'_prob.pdf')
        ...
        logp = np.empty(nReruns)
        
        for iRerun in range(nReruns):
            SSErhoL[iRerun], SSEPsat[iRerun], SSErhov[iRerun], SSEU[iRerun], SSEP[iRerun], SSEZ[iRerun], Neffavg[iRerun], logp[iRerun] = analyze_ITIC(iRerun,use_PCFR,metric)
